 ...
 * entry types map onto them. Capture code (the consumer's observers — see examples/capture) uses
 * this to route entries to streams; any reader uses it to enumerate streams. Keep this in lockstep
 * with the per-stream model types in `streams/` and with STREAM_SCHEMA_VERSIONS in `version.ts`.
 */
...
export const STREAM_IDS = [
  ...
  'profile',
  'errors',
  // Appended last so existing stream indices stay stable (indices key section headers + the config/
  // overhead maps). Not observer-sourced — fed explicitly via the Encoder, like profile/environment.
  'customEvents',
] as const;
 ...
 * `environment` (navigator), `profile` (Profiler API), `errors` (window events), and `customEvents`
 * (explicit app instrumentation) are not PerformanceObserver entry types. Several entry types fold
 * into one stream: `first-input` + `event`
